...
class ImageUtils:
    @staticmethod
    def load_dataset(dataset_path, img_size, class_mode='categorical', train_batch_size=32, test_batch_size=32):
        ...
        test_dir = ImageUtils.prepare_test_dir(dataset_path)
    ...
    @staticmethod
    def prepare_test_dir(dataset_path):
        test_dir = os.path.join(dataset_path, 'Test_prepared')
        ...
        with open(os.path.join(dataset_path, 'Test.csv'), 'r') as file:
            ...
            for line in lines:
                ...
                img_path = parts[-1]
                class_id = parts[-2]
                shutil.copy(os.path.join(dataset_path, img_path), os.path.join(test_dir, class_id))
    ...
    def generate_dataset(dataset_path, img_size, class_mode='categorical'):
        batch_size = 128
        generator, _ = ImageUtils.load_dataset(dataset_path, img_size, class_mode=class_mode)

        number_imgs = ImageUtils.calculate_recursively_number_imgs(os.path.join(dataset_path, "Train"))
    ...
    def calculate_recursively_number_imgs(path):
        n_imgs = sum([len(files) for r, d, files in os.walk(path)])
        return n_imgs
    ...
    def save_images(imgs, path_to_save):
        ...
        for img in imgs:
            ...
            cv2.imwrite(os.path.join(path_to_save, f'{index}.png'), img)
            index += 1
...
class ModelUtils:
    ...
    def load_model(file_model_path):
        ...
        if not file_model.is_file():
            return None
        model = tf.keras.models.load_model(file_model_path)
        return model
...
class FSGMAttack:
    ...
    def train(self, dataset, n_epochs, max_step, n_epsilons=10):
        ...
        for e in epsilons:
            print(f"[*] Training algorithm for epsilon = {e}")
...
def train(data_path, img_size, n_epochs, path_save, batch_size=2, n_epsilons=10):
    number_imgs = ImageUtils.calculate_recursively_number_imgs(os.path.join(data_path, "Train"))
    max_step = (number_imgs // batch_size) + 1 if number_imgs % batch_size != 0 else number_imgs // batch_size
    ...
    fsgm_attack.save_model(os.path.join(path_save, 'new_model.h5'))
...
def generate_modified_imgs(data_path, epsilon, img_size, path_save, batch_size=2):
    ...
    for imgs, labels in dataset.take(max_step):
        ...
        print(f"     [*] Image generated: (Number: {modified_imgs.shape[0]}, height: {modified_imgs.shape[1]}, width: {modified_imgs.shape[2]})")
...
parser.add_argument('dataset_path', type=str, help='Enter the dataset path. Each of the images must be in a folder that indicates its label')
...
parser.add_argument('-e', '--epsilon', type=float, default=0.1, help='Enter how much you want to modify the images. If epsilon is small, the modifications of images will be small too. This argument is only needed for task 1 and 2')
parser.add_argument('-b', '--batch-size', type=int, default=1, help='Enter the batch size. For efficiency reasons, it should be a multiple of 2. For example: 16, 32, 64, 128')
parser.add_argument('-n', '--n_epochs', type=int, default=15, help='Introduce the number of epochs you want to train the neural network. This argument is only needed for task 3')
parser.add_argument('-v', '--epsilon_values', type=int, default=10, help='Enter how many epsilons you want to generate to train the model. This argument is only needed for task 3')
...
print(f"[*] Devices detected: {tf.config.list_physical_devices()}")
...
if args.task == 'gen_data':
    ...
    print(f"[+] Images generated in {args.results_path}")
    ...
    print(f"[+] Your model has a loss rate of {per_val}%")
    print(f"[+] Your model has a loss rate of {per_mod}% with images modified (using FGSM attack)")
